...
- **Incremental finals via append-only partials.** Nemotron's partial hypothesis only ever grows — it does not revise committed text. Finals are derived by forwarding each stable extension of the partial string at commit boundaries (last whitespace; for spaceless scripts like zh, CJK sentence punctuation, then a length-threshold backstop — whitespace boundaries never arrive there). No engine API change needed; the existing `onPartial` callback carries everything.
