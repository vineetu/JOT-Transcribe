...
- **Post-processing** — transcripts run through a deterministic cleanup chain before delivery. What applies depends on the model, since v2 emits rawer text than v3 / Nemotron:
  - **Filler-word removal + recapitalization** — regex strip of `um/uh/er/uhm/erm`, no LLM. **v2 only** — the recapitalization is what v3 / Nemotron don't need (they emit correct casing), so running it on their output would regress it.
  - **Number normalization** — deterministic spoken-number → digit conversion (handles money, percent, years — including bare "twenty twenty-four" and century forms — decimals ("three point one four" → 3.14), dates ("May fourth" → May 4th), duration homographs ("thirty second timeout" → 30-second, not 30th), time-of-day, address, cardinals; preserves idioms and phone-shaped sequences). Casing-safe, so it now runs on **English** transcripts from v2, Nemotron, and the v3 English / Nemotron-paired paths. Its spelled-cardinal rules are English-only, so non-English European (v3), Japanese, and the non-Latin Nemotron-multilingual languages are excluded.
  - **Paragraph segmentation** — pause-based `\n\n` breaks, applied to any model that returns FluidAudio token timings (v2 and v3). Nemotron returns no per-word timings, so its transcripts stay a single block.
