...
- **Language-based model selection** — you pick the **language** you speak (Settings → Transcription or the Setup Wizard); Jot auto-selects and downloads the right on-device model. Model names are hidden — they surface only in About → Acknowledgements. All run via FluidAudio on the ANE; switching is non-destructive (other models stay installed unless you delete them):
  ...
  - **Regional variants (locales)** — English, Spanish, French, and Portuguese each offer two picker entries: **English (US) / English (UK)**, **Spanish (Latin America) / Spanish (Spain)**, **French (France) / French (Canada)**, **Portuguese (Brazil) / Portuguese (Portugal)**. Both members of a pair resolve to the same model — no extra download; the locale is passed to the Nemotron multilingual engine as its per-locale prompt. Measured effects: Spanish (Spain) tends to render some numbers and dates as digits ("1250 euros", "el 5 de noviembre") where Spanish (Latin America) spells them out; French (France) vs French (Canada) differ in orthography ("week-end" vs "weekend", numeral hyphenation). Pre-locale users keep working unchanged — stored "Spanish" maps to Latin America, "French" to France, "Portuguese" to Brazil, "English" to US. The system-locale default picks the matching variant (e.g. a UK-region Mac defaults to English (UK)). On the Parakeet v3 path both members of a pair behave identically.
...
- **Post-processing (runs for every model, language-aware)** — every transcript runs through a deterministic cleanup chain before delivery. One code path; per-language behavior: English gets the full chain; **Spanish / German / French / Italian / Portuguese get hesitation-filler cleaning** (es *eh/em*, de *äh/ähm/öh/hm*, fr *euh*, it *ehm/mmm*, pt *hum* — non-lexical sounds only, never real words, and **no recapitalization** so the model's own casing plus abbreviations like "z. B." / "M. le président" survive; Spanish paired `¿…?` punctuation is preserved). **Number normalization stays English-only** — its rules are English-hardcoded and would mis-convert other languages (French "six cents" = 600 → "6¢"); the multilingual models emit digits natively where it matters (measured — see `../jot-shared/docs/multilingual-itn-options.md`). A **model-artifact scrub** removes Nemotron `<unk>` tokens (its tokenizer has no %/€/$, so Spanish "veinticinco por ciento" used to paste as literal `25<unk>` — now `25%`), on finals and live preview both. Paragraph segmentation and whitespace cleanup are language-agnostic and run for all languages:
...
- **`jot` command-line tool** — a bundled CLI (`Jot.app/Contents/Helpers/jot-cli`) that transcribes any audio/video file to **WebVTT** from the terminal, with an optional `--diarize` for speaker labels: `jot-cli transcribe meeting.mp4 --diarize -o out.vtt`. Reuses the same on-device engine + bundled ffmpeg + the app's downloaded models — nothing leaves the Mac, no network. This is the honest home for diarization: feed it a clean meeting recording, get labeled, timestamped turns out. Install it on your PATH with `brew install --cask vineetu/jot/mac`, which puts it there as `jot-cli`.
...
- **`jot` CLI** — a bundled, self-contained command-line transcriber (`Contents/Helpers/jot-cli`, signed + hardened runtime like the ffmpeg helper) reusing the on-device engine → WebVTT, `--diarize` for word-aligned speaker cues. Standalone SPM package `tools/jot-cli/`. Design: `docs/jot-cli/design.md`.
